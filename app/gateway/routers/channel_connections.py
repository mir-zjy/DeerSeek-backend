...
async def _get_runtime_config_store(request: Request) -> ChannelRuntimeConfigStore:
    ...
    if isinstance(store, ChannelRuntimeConfigStore):
        return store
    # 构造 store 会从磁盘读取 JSON 文件，放到线程中执行以免阻塞事件循环。
...
async def _create_state(
    ...
    *,
    ...
    # 原子化执行“删除过期 + 计数 + 插入”，防止同一 owner 的并发 connect 请求
    # 都看到 count < 上限而全部插入、超出上限。
...
def _provider_response(
    ...
    if unavailable_reason is not None:
        # 运行时 provider 不可用，因此过期的 “connected” 记录不能被报告为
        # 已连接。其他状态（如 “revoked”）保留，以便调用方仍能区分
        # “已吊销的绑定”与“从未连接”。
        ...
        # 本地免鉴权模式下所有渠道消息都路由到默认用户，
        # 因此已配置且正在运行的渠道无需按用户绑定。
...
async def get_channel_providers(request: Request) -> ChannelProvidersResponse:
    ...
    # 各 provider 的就绪检查相互独立，并发执行，避免某个渠道的
    # 慢速重启拖慢整个 /providers 响应。
...
async def disconnect_channel_provider_runtime(provider: str, request: Request) -> ChannelProviderResponse:
    ...
    # 在提交 store/缓存之前先吊销 DB 中的连接记录，避免 repo 失败导致
    # store 和缓存显示“已断开”而 DB 仍保留“connected”记录，
    # 之后重新配置时这些记录会被静默重新激活。
    if repo is not None:
    ...
    # 重新读取实时缓存配置，仅移除该 provider，以免覆盖其他 provider 的
    # 并发修改。此次读取与重新赋值之间不允许出现 await。
...
async def configure_channel_provider_runtime(
    ...
    if provider == "telegram":
        # deep-link 用户名随运行时渠道配置持久化（见下方
        # set_provider_config），并通过 apply_runtime_connection_config
        # 应用到后续请求；切勿修改 get_app_config() 缓存的配置实例。
    ...
    # 重新读取实时缓存配置，仅应用该 provider 的变更，以免覆盖其他
    # provider 的并发修改。此次读取与重新赋值之间不允许出现 await。
    live_channels_config = await _get_channels_config(request)
