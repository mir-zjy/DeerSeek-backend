...
# 匹配完整的 <think>...</think> 块（不区分大小写，可跨行）。
_THINK_BLOCK_RE = re.compile(r"<think\b[^>]*>.*?</think\s*>", re.IGNORECASE | re.DOTALL)
# 匹配悬空未闭合的 <think>（模型在 max_tokens 处被截断，思考未结束）。
_OPEN_THINK_RE = re.compile(r"<think\b[^>]*>", re.IGNORECASE)


def _strip_think_blocks(text: str) -> str:
    """移除响应中推理模型的 ``<think>...</think>`` 块。

    MiniMax-M3 等推理模型会把思维链内联到消息的 ``content`` 中，
    用 ``<think>...</think>`` 包裹（``reasoning_split`` 默认为 false），
    而不是提供独立的 ``reasoning_content`` 字段。思考文本中常含有
    ``[`` / ``]`` 字符，会干扰下游基于 ``find('[')`` / ``rfind(']')``
    的 JSON 提取，导致生成空建议。因此在解析前先剥离推理内容，
    只保留实际回答。
    """
    text = _THINK_BLOCK_RE.sub("", text)
    # 丢弃截断残留的未闭合 <think>（及其后的所有内容）。
